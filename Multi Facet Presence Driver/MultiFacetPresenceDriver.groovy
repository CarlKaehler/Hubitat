 ...
 *  Changes:
 *
 *  1.0.3 - 12/27/21 - Modified to Compgeek
...
metadata {
	definition (name: "Multi Facet Presence Driver", namespace: "Compgeek", author: "Carl Kaehler") {
        capability "Actuator"
	...
	preferences() {    	
        section(){
            input name: "about", type: "paragraph", element: "paragraph", title: "<b>Compgeek Presence</b>", description: "Combination Presence and on/off device."
...
def on() {
    if(logEnable) log.info "In Compgeek Presence Driver - Turning Switch On"
    device.on
    def now = new Date()
...
def off() {
    if(logEnable) log.info "In Compgeek Presence Driver - Turning Switch Off"
    device.off
    def now = new Date()
