    ...
    dynamicPage(name: "mainPage") {
		...
		if(state.appInstalled == 'COMPLETE'){
			section("${getImage('instructions')} <b>Instructions:</b>", hideable: true, hidden: true) {
				...
				paragraph "<div style='color:#1A77C9'>Use Color Bulbs to Notify of Commute Time</div>"
			}
			section(getFormat("header-blue", "${getImage("Blank")}"+" Child Apps")) {
				app(name: "anyOpenApp", appName: "Commute Time Child", namespace: "Compgeek", title: "<b>Add a new 'Commute Time' child</b>", multiple: true)
			}
			section(getFormat("header-blue", "${getImage("Blank")}"+" General")) {
       			label title: "Enter a name for parent app (optional)", required: false
                input "apiKey", "text", title: "Google API Key", required: true, displayDuringSetup: true
 			}
			display2()
		}
	}
